        ...
        try {
            ...
            while(br.ready()) {
                ...
                for (String textNode : textRow) {
                    ...
                    switch (textNode) {
                        case VACUUM_CHAR:
                            gridNode = new GridNode(x, y, grid, new Pair<>(PASS_OF_VACUUM, INVCAP_OF_VACUUM), new Tile(VACUUM_CHAR, null));
                            break;
                        case STEELBULKHEAD_CHAR:
                            gridNode = new GridNode(x, y, grid, new Pair<>(PASS_OF_STEELBULKHEAD, null), new Tile(STEELBULKHEAD_CHAR, null));
                            break;
                        case STEELBULKHEADDOOR_CHAR:
                            gridNode = new GridNode(x, y, grid, new Pair<>(PASS_OF_STEELBULKHEADDOOR, INVCAP_OF_STEELBULKHEADDOOR), new Tile(STEELBULKHEADDOOR_CHAR, null));
                            break;
                        case STEELFLOORBULKHEAD_CHAR:
                            gridNode = new GridNode(x, y, grid, new Pair<>(PASS_OF_STEELFLOORBULKHEAD, INVCAP_OF_STEELFLOORBULKHEAD), new Tile(STEELFLOORBULKHEAD_CHAR, null));
                            break;
                        default:
                            gridNode = null;
                            break;
                    }
                    gridNode.getTile().setLocation(gridNode);
                    row.add(gridNode);
                }
                grid.add(row);
            }
